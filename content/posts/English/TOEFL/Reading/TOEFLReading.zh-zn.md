...
文章数量：3~4篇/ 54~72mins/ 30Q~40Q（存在加试）
...
![](https://file.koolearn.com/80201646808311.png)

2021年过去一年内统计到的332篇文章：

- 其中**生命科学类（生物）和历史类**的话题文章占比非常高，分别为40%和22%；
- 其次，**社会类、考古类**的文章分别以13%和11%位居第三、第四；
- **地理类文章**以8%的占比位居第五；
- **艺术类和天文科学类**文章分别以3%排在末位。
...
高效阅读 = Skimming >> scanning，考试中略读为主，精读为辅。
...
**一篇阅读的整体做题步骤伪代码**：以段落为核心突破点

```
invoke function 结构式阅读法(p1);
read.solve(P1.Q1);
read.solve(P1.Q2);
read.solve(P1.Q3)...

invoke function 结构式阅读法(P2);
read.solve(P2.Q1);
read.solve(P2.Q2);
read.solve(P2.Q3)...

invoke function 结构式阅读法(P3);
read.solve(P3.Q1);
read.solve(P3.Q2);
read.solve(P3.Q3)...
