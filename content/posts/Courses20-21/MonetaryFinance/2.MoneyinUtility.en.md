...
where $z_t $ is the flow of services yielded by money holdings and $ c_t $ is time t per capita consumption. $ \lim_{z \rightarrow 0} u_{z}(c, z)=\infty $ for all $ c, $ where $ u_{z}=\partial u(c, z) / \partial z $.
...
where $Y_t$ is aggregate output, $K_t$is the aggregate stock of capital at the start of period t, $\delta$ is the rate of depreciation of physical capital,  $\tau_t N_t$ is the aggregate real value of any lump-sum transfers or taxes, $B_t$ is the amount of bonds, $i_t$ is nominal interest rate.
...
$$
y_{t}=f\left(\frac{k_{t-1}}{1+n}\right) \tag{2.3}
$$
...
$$
 i_{t}=r_{t}+\pi_{t+1} 
$$
To interpret (2.12) , consider a very simple choice problem in which the agent must pick $ x $ and $ z $ to maximize $ u(x, z) $ subject to a budget constraint of the form $ x+p z= $ $ y $, where $ p $ is the relative price of $ z $. The first-order conditions imply $ u_{z} / u_{x}=p $; in words, the marginal rate of substitution between $ z $ and $ x $ equals the relative price of $ z $ in terms of $ x $.
